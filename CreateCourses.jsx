...
function CreateCourses() {
    ...
    const handleCreateCourses = async ()=> {
        setLoading(true)
        try {
            const result = await axios.post(serverUrl + "/api/course/create" , {tital,category},{withCredentials:true})
    ...
    <div className='min-h-screen flex items-center justify-center bg-gray-100 px-4 py-10'>
        <div className='max-w-xl w-[600px] mx-auto p-6 bg-white shadow-md rounded-md mt-10 relative'>
            ...
            <form  className='space-y-5'onSubmit={(e)=>e.preventDefault()}>
                <div>
                    <label htmlFor="tital" className='block text-sm font-medium text-gray-700 mb-1'>Course Title</label>
                    <input type="text" id='title' placeholder='Enter Course title' className='w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:right-2 focus:ring-[black]' 
                ...
                <button className='w-full bg-[black] text-white py-2 px-4 rounded-md active:bg-[#3a3a3a] transition' disabled={loading} onClick={handleCreateCourses}>{loading? <ClipLoader size={30} color='white'/>:"Create"}</button>
