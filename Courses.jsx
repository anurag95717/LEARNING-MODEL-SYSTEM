...
function Courses() {
    ...
    const {creatorCourseData}= useSelector(state=>state.Courses)
  return (
    <div className='flex min-h-screen bg-gray-100'>
        <div className='w-[100%] min-h-screen p-4 sm:p-6 bg-gray-100'>
        <div className=' flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-3'></div>
        {/* for Large Screen table */}
        <div className='flex items-center justify-center gap-3'>
            <FaArrowLeftLong className='w-[22px] h-[22px] cursor-pointer' onClick={()=>navigate("/dashboard")}/>
                <h1 className='text-2xl font-semibold'>All Created Courcses</h1>
        </div>
        <button className='bg-[black] text-white px-4 py-2 rounded hover:bg-gray-500' onClick={()=>navigate('/createcourse')}></button>
                ...
                <tbody>
                    ...
                    <tr key={index} className='border-b hover:bg-gray-50 transition duration-200'>
                        <td className='py-3 px-4 flex items-center gap-4'>
                            {Course?.thumbnail ?<img src={Course?.thumbnail}  className="w-25 h-14 object-cover rounded-md  " alt="" />:<img src={img} className='w-25 h-24 object-cover rounded-md' alt=''/>}<span>{course?.title}</span>
                        </td>
                       {course?.price ? <td className='px-4 py-3 '>{course?.price}</td>
                        ...
                        <td className='px-4 py-3'>
                            <FaRegEdit className='text-gray-600 hover:text-blue-600 cursor-pointer'/>
